...
| The v1.2 `C_4` exclusion-witness table gives one rational uncovered-gap witness for each of the `208` residues outside `{2,208} mod 210` | Exact generated artifact |
...
| The finite `B_5` proposition states the exact birth set `{118,448,542,778,849,872,1108,1202,1438,1461,1532,1768,1862,2192} mod 2310`; the v1.4 pair summary compresses it into `7` reflection-pair rows for the displayed table | Exact generated artifact |
...
| The v0.12 direct `C_8` anchor-neighborhood table shows that `4`, `206`, and `201` are clean `C_4` boundary classes, while `111`, `99`, `118`, `88`, and `62` are mixed shallow-anchor neighborhoods | Experiment / diagnostic |
...
- This project does not ask whether `|C_k|/M_k` decays; lift monotonicity makes
  ...
  the monotone limit.
- This project does not claim `C_4={2,208}` has a fully handwritten 208-case
  exclusion proof yet; the exclusion is currently presented as a human-readable
  finite appendix backed by exact row-level rational witnesses, tests, the v1.7
  low-level rational CSV verifier, and the v1.8 standard-library standalone
  checker.
...
- This project does not claim the v0.11 lift-boundary table explains complete
  ...
  of shallow anchors.
- This project does not claim the v0.12 direct `C_8` anchor-neighborhood table
  explains complete covering; it separates residue-ring geometry from observed
  sample weighting.
