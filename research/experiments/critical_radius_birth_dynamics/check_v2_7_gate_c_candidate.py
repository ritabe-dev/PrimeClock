#!/usr/bin/env python3
"""Audit the v2.7 Gate C candidate bundle and ZIP."""
...
MANIFEST_REL = EXPERIMENT_REL / "prc_v2_7_gate_c_candidate_manifest_v0_1.json"
THEOREM_CHECKER_REL = (
...
EXPECTED_ARTIFACTS = {
    ...
    "theorem_checker": str(THEOREM_CHECKER_REL),
    "exact_audit": str(EXACT_AUDIT_REL),
    "gate_c_checker": str(EXPERIMENT_REL / "check_v2_7_gate_c_candidate.py"),
    "workflow": str(EXPERIMENT_REL / "candidate_workflow_v2_7_v0_1.yml"),
    "bundle_builder": str(EXPERIMENT_REL / "candidate_bundle.py"),
    "bundle_manifest": str(
        EXPERIMENT_REL / "gate_c_candidate_bundle_manifest_v2_7_v0_1.json"
    ),
    "bundle_profile": "v2_7_gate_c_candidate",
    "bundle_name": "PrimeClock-v2.7-gate-c-candidate-v0.1",
    "support_csv": str(
        EXPERIMENT_REL / "data/prc_prime_prefix_birth_dynamics_k5_k7_v0_1.csv"
    ),
}
REQUIRED_BOUNDARIES = (
    "not_public_release",
...
REQUIRED_BUNDLE_FILES = (
    ...
    "research/experiments/critical_radius_birth_dynamics/candidate_workflow_v2_7_v0_1.yml",
    ...
    "research/experiments/critical_radius_birth_dynamics/notes/prc_v2_7_gate_c_candidate_readme_v0_1.md",
    "research/experiments/critical_radius_birth_dynamics/notes/prc_v2_7_gate_c_candidate_verify_v0_1.md",
    "research/experiments/critical_radius_birth_dynamics/notes/prc_v2_7_general_single_gap_aperture_theorem_note_v0_1.md",
    "research/experiments/critical_radius_birth_dynamics/prc_v2_7_gate_c_candidate_manifest_v0_1.json",
    "research/experiments/critical_radius_birth_dynamics/gate_c_candidate_bundle_manifest_v2_7_v0_1.json",
...
def require_manifest(data: dict[str, object], repo_root: Path, failures: list[str]) -> None:
    expected_scalars = {
        "candidate_id": "prc_v2_7_general_q_prime_single_gap_aperture_theorem_candidate",
        "gate": "gate_c_candidate",
        "gate_r_status": "closed_reviewed_candidate",
    ...
    if not isinstance(artifacts, dict):
        ...
        for key, expected in EXPECTED_ARTIFACTS.items():
            ...
            elif key in {"bundle_profile", "bundle_name"}:
                continue
    ...
    if not isinstance(reproducibility, dict):
        ...
        expected = {
            "primary_artifact": "theorem_note",
            "theorem_scope": "general_q_prime_prc_circular_arc_model",
            "exact_audit": "implementation_consistency_not_geometric_proof",
            "support_csv": "minimal_existing_checked_scope_csv_only",
...
def run_script(repo_root: Path, relative_path: Path, label: str, failures: list[str]) -> None:
...
def check_bundle_root(bundle_root: Path, failures: list[str]) -> None:
    ...
    run_script(bundle_root, THEOREM_CHECKER_REL, "v2.7 theorem-note checker", failures)
    run_script(bundle_root, EXACT_AUDIT_REL, "v2.7 exact audit", failures)
    run_bundle_builder_check(bundle_root, failures)


def expected_zip_names_from_manifest(
    archive: zipfile.ZipFile,
    root_name: str,
    failures: list[str],
) -> set[str]:
    manifest_name = f"{root_name}/BUNDLE_FILE_MANIFEST.txt"
    if manifest_name not in archive.namelist():
        failures.append("Gate C candidate ZIP missing BUNDLE_FILE_MANIFEST.txt")
        return set()
    raw = archive.read(manifest_name).decode("utf-8")
    expected = {manifest_name, f"{root_name}/SHA256SUMS"}
    for line_number, line in enumerate(raw.splitlines(), start=1):
        ...
        if pure_path.is_absolute() or any(part in {"", ".", ".."} for part in pure_path.parts):
            failures.append(
                f"Gate C candidate ZIP has unsafe BUNDLE_FILE_MANIFEST path "
                f"on line {line_number}: {relative!r}"
            )
            continue
        expected.add(f"{root_name}/{relative}")
    return expected
...
def check_bundle_zip(zip_path: Path, failures: list[str]) -> None:
    ...
    with archive:
        ...
        if root_name != EXPECTED_ARTIFACTS["bundle_name"]:
            failures.append(
                f"Gate C candidate ZIP top-level directory must be "
                f"{EXPECTED_ARTIFACTS['bundle_name']!r}"
            )
        for name in names:
        ...
        expected_names = expected_zip_names_from_manifest(archive, root_name, failures)
        ...
        for relative in REQUIRED_BUNDLE_FILES:
            ...
            if expected_name not in actual_names:
                failures.append(f"Gate C candidate ZIP missing required file {expected_name}")
        if failures:
            return
        with tempfile.TemporaryDirectory(prefix="prc-v27-zip-check-") as temp_dir:
            ...
            if failures:
                return
            check_bundle_root(temp_root / root_name, failures)


def check_reproducible_zip(repo_root: Path, failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="prc-v27-repro-") as temp_dir:
        temp_root = Path(temp_dir)
        zip_paths: list[Path] = []
        for label in ("a", "b"):
...
def check_negative_cases(bundle_root: Path, zip_path: Path, failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="prc-v27-negative-") as temp_dir:
        temp_root = Path(temp_dir)

        broken_bundle = temp_root / "broken-bundle"
        shutil.copytree(bundle_root, broken_bundle)
        bad_note = "# Broken note\n\nThis intentionally lacks the required theorem-note structure.\n"
...
def main() -> int:
    ...
    if args.zip_only:
        ...
        if args.bundle_root is not None:
            ...
            run_script(repo_root, THEOREM_CHECKER_REL, "v2.7 theorem-note checker", failures)
            run_script(repo_root, EXACT_AUDIT_REL, "v2.7 exact audit", failures)
        if args.zip is not None:
            check_bundle_zip(args.zip.resolve(), failures)
        if args.reproducible_zip:
