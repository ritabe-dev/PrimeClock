...
- `check_candidate_standalone.py`, a standard-library-only checker that reads
  the committed CSV artifacts, verifies the candidate CSV SHA256 manifest, and
  checks the headline finite claims without importing `prime_reciprocal_projection`
  or `experiments/critical_radius_birth_dynamics/tools.py`.
